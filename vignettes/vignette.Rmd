...
Positions and sizes of hurricane Katrina (2005), Sandy (2012), Harvey (2017), and Ian (2022) when they made their strongest landfall.
```{r}
...
Data frame containing information for each individual storm.
```{r}
...
ggplot( data = storm.name.dat, aes(x=name, y=n, label=name, color=n) ) +
    geom_text(size=2, angle=45, family="Times New Roman") +
...
The current policy of retiring hurricane names is based on a rotating six-year list, which was implemented by the World Meteorological Organization (WMO) in 1979. 


\newpage

The distances traveled in miles by each storm per year was also looked at. The distance was computed by taking the storm center's interpolated longitude and latitudes and summing the distance between each point. The distances were averaged between each storm, and, as shown below, revealed that the distance is increasing with time. Linear regression was used to look at the relationship presenting the model: $Y_{i} = -4522.249 + 3.519*year$.
```{r}
...
ggplot(data=distance.df) +
  ...
  labs(x="Year", y="Average Distance",
       title="Average Distance Hurricane Travels per Year", color="",
...
landfall.hist <- ggplot(data=landfall.dat ) +
    geom_histogram(aes(x=year, y=freq, fill=cut(freq, 12)), stat="identity") +
    labs(x="Year", y="Landfall", title="Landfalling Hurricanes per Year (1851-2022)", fill="Frequency") +
    theme_light() + 
    scale_y_continuous(n.breaks=7) + scale_x_continuous(n.breaks = 14)
landfall.hist
```
...
ggplot( data=landfall.dat ) +
    ...
    geom_smooth(aes(x=year, y=total.storms), method=lm, se=FALSE, group=1, colour="grey40", alpha=0.1) +
    labs(x="Year", y="Hurricanes", title="Hurricanes per Year (1851-2022)", color="Total Hurricanes") +
    scale_y_continuous(n.breaks=7) + scale_x_continuous(n.breaks=14) +
    theme_light() + theme(panel.grid.minor = element_blank())
```

Other factors contributing to this growing amount of landfalling hurricanes could be climate change, improved technology, population growth and urbanization, and natural climate variability. Unfortunately, they can not be analyzed due to the limitations of the information within the hurdat dataset.
...
ggplot(data=cyclone.dat ) +
    ...
    labs(x="Year", y="Accumulated Cyclone Energy", title="Average ACE per Year (1851-2022)") +
    scale_y_continuous(n.breaks=7) + scale_x_continuous(n.breaks=14) +
    theme_light() + theme(panel.grid.minor = element_blank())
```
...
```{r}
ggplot(data=cyclone.dat ) +
    stat_function( fun=function(year) -654.78611 + 0.38882*year, aes(color="Regression Line") ) + 
    geom_point(aes(x=year, y=ace, fill="Average ACE")) +
    scale_y_continuous(n.breaks=7) + scale_x_continuous(n.breaks=14) +
    theme_light() + theme(panel.grid.minor = element_blank()) +
    labs(x="Year", y="Accumulated Cyclone Energy", title="Average ACE per Year (1851-2022)",
         colour="", fill="")
