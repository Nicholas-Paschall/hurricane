...
cyclone_energy <- function( storm, year, dat ){
    ...
    if( !(storm %in% c(dat$name, dat$id) ) ){ stop( "Hurricane not found." ) }
    if( storm=="UNNAMED" ){ stop( "UNNAMED hurricanes require storm ID." ) }
    if( !grepl("AL\\d+", storm) ){
        storm <- dplyr::filter(dat,
            name==storm & grepl(year, date.time))[1,]$id
        if( is.na(storm) ){ stop( "Incorrect year for hurricane.")}
    }

    # compute ACE based on following conditions
    storm_dat <- subset(dat, id==storm & year==year &
                            max.wind >= 35 &
                            grepl("0000|0600|1200|1800", dat$UTC.time))
