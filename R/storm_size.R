#' Plots the positions and sizes of given storms.
#'
...
#' longitude, and storm size is determined by its wind radii at 34 knots,
#' 50 knots, and 64 knots.
#'
...
#' @param date.time.list vector of date and time values in the
#' format: Y-m-d H:M:S UTC
#' @param path logical. If TRUE (default), storm path is plotted
#' @return ggplot object displaying map along with position and size of
#' of storm. A red circular shape represents radii of wind at 64 knots. An
#' orange circular shape represents radii of wind at 50 knots. A green circular
#' shape represents radii of wind at 34 knots.
#'
#' @examples
#' size1 <- storm_size("KATRINA", 2005, hurdat, "2005-08-28 06:00:00 UTC")
#' size2 <- storm_size("KATRINA", 2005, hurdat, "2005-08-28 06:00:00", path=FALSE)
#' size3 <- storm_size(c("KATRINA", "SANDY"), c(2005, 2012), hurdat, c("2005-08-28 06:00:00", "2012-10-25 18:00:00"), path=TRUE)
#' size4 <- storm_size(c("AL122005", "AL182012"), c(2005, 2012), hurdat, c("2005-08-25 18:00:00", "2012-10-28 12:00:00"), path=FALSE)
#'
#' @export
storm_size <- function( storm.list, year.list, dat, date.time.list, path=TRUE ){

    # obtain interpolated latitude and longitudes for each storm
