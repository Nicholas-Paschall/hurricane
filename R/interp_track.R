...
#'    \item{date.time}: date and time throughout storm's duration in thirty
#'    minute increments.
#'    \item{record.id}: used to identify records that correspond to landfalls or
#'    to indicate the reason for inclusion of a record not at the standard
#'    synoptic times.
#'    \item{latitude}: interpolated latitude of storm's center in degrees north.
#'    \item{longitude}: interpolated longitude of storm's center. Negative
#'    values indicate west longitudes, positive values indicate east longitudes.
#'    \item{id}: identification code of the chosen storm.
#'    \item{storm.status}: status of storm, referring to level of
#'    intensity.
#'    \item{max.wind}: the maximum 1-min average wind (in knots) associated
#'    with the tropical cyclone at an elevation of 10 m with an unobstructed
#'    exposure.
#'}
#'@examples
...
interp_track <- function( storm, year, dat ){
    ...
    # interpolate latitude and longitudes for new times
