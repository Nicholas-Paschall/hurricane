...
storm_size_interp_34 <- function( center.point, NE.point, SE.point, SW.point, NW.point ){
    ...
    for( i in seq_along(S.quadrant.theta) ){
        dist.theta.S[i] <- dist.SE + (( S.quadrant.theta[i] - S.quadrant.theta[1] )/
    ...
    for( i in seq_along(W.quadrant.theta) ){
        dist.theta.W[i] <- dist.SW + (( W.quadrant.theta[i] - W.quadrant.theta[1] )/
    ...
    for( i in seq_along(N.quadrant.theta) ){
        dist.theta.N[i] <- dist.NW + ((N.quadrant.theta[i] - N.quadrant.theta[1] )/
    ...
    for( i in seq_along(E.quadrant.theta) ){
        dist.theta.E[i] <- dist.NE + ((E.quadrant.theta[i] - E.quadrant.theta[1] )/
...
storm_size_interp_50 <- function( center.point, NE.point, SE.point, SW.point, NW.point ){
    lon.rad <- center.point[1] * pi/180
    lat.rad <- center.point[2] * pi/180
    dist.NE <- fields::rdist.earth(
        matrix(center.point, ncol=2), matrix(NE.point, ncol=2), miles=F, R=6371)
...
storm_size_interp_64 <- function( center.point, NE.point, SE.point, SW.point, NW.point ){
    lon.rad <- center.point[1] * pi/180
    lat.rad <- center.point[2] * pi/180
    dist.NE <- fields::rdist.earth(
        matrix(center.point, ncol=2), matrix(NE.point, ncol=2), miles=F, R=6371)
