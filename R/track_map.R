...
#' @return A ggplot object displaying the world map along with color coded
#' storm tracks or specific locations of each inputted storm.
#' @examples
...
#' date.time.list = c("2005-08-23 18:00:00",
#' "2012-10-21 18:00:00 UTC"))
...
track_map <- function( storm.list, year.list, dat, date.time.list=NULL ){
    ...
    # if date.time.list not given, function generates track of each storm iteratively
    if( is.null(date.time.list) ){
        ...
        for( i in seq_along( unique( storm_dat$id ) ) ){
            ...
            map <- map + ggplot2::geom_path(data=tmp.dat,
                ggplot2::aes(x=longitude, y=latitude, color=labels), lwd=1.5)
        }
        map <- map + ggplot2::labs(color="Storm")
    }

    # if date.time.list given, function generates points of each date.time.list iteratively
    else{
